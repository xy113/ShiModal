...
function U($params=null){
	if (is_null($params)) {
		...
		$url.= isset($arr['c']) ? '&c='.$arr['c'] : '&c='.G('c');
		$url.= isset($arr['a']) ? '&a='.$arr['a'] : '&a='.G('a');
 ...
 */
function getLocationByCity($city='',$address=''){
	$location = array();
...
function getLocationByIP($ip=null){
	...
	cookie('location_ip',null);
	$location = cookie('location_ip');
	$location = unserialize($location);
	if ($location && $location['ip'] == $ip){
		return $location;
	}else {
		$url = 'http://restapi.amap.com/v3/ip?key=389880a06e3f893ea46036f030c94700&ip='.$ip;
		$json = httpGet($url);
		$data = json_decode($json,true);
		if ($data['info'] == 'OK'){
...
function getLocationByPoint($lng,$lat){
	...
	$lat = floatval($lat);
	$point = $lng.','.$lat;
	$url = 'http://restapi.amap.com/v3/geocode/regeo?key=389880a06e3f893ea46036f030c94700&extensions=base&location='.$point;
	$json = httpGet($url);
	$data = json_decode($json,true);
	$location = array();
	if ($data['info'] == 'OK'){
 ...
 * @return string
 */
function locality($name='city', $value=null){
	if (is_null($value)) {
		$locality = cookie($name);
		if ($locality) {
			return $locality;
		}else {
			$data = getLocationByIP();
			if ($name == 'city') {
				$locality = $data['city'];
			}else {
				$locality = $data['province'];
			}
			cookie($name, $locality);
			return $locality;
		}
	}else {
		cookie($name, $value);
