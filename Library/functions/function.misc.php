 ...
 */
function image_get_list($condition, $num=20, $limit=0, $order=''){
	$limit = $num ? "$limit, $num" : ($limit ? $limit : '');
	!$order && $order = 'id ASC';
	$piclist = M('image')->where($condition)->limit($limit)->order($order)->select();
	if ($piclist) {
		$datalist = array();
 ...
 * @return unknown|unknown
 */
function media_add_data($data, $return=0){
	$id = M('media')->insert($data, true);
	if ($return) {
		return media_get_data(array('id'=>$id));
	}else {
		return $id;
	}
}
 ...
 */
function media_delete_data($condition){
	if ($condition) {
		return M('media')->where($condition)->delete();
	}else {
		return false;
	}
}
 ...
 */
function media_update_data($condition, $data){
	return M('media')->where($condition)->update($data);
}
 ...
 * @param mixed $condition
 */
function media_get_data($condition){
	$data = M('media')->where($condition)->getOne();
	if ($data) {
		return $data;
	}else {
		return array();
 ...
 * @return mixed
 */
function media_get_num($condition, $field='*'){
	return M('media')->where($condition)->count($field);
}
 ...
 * @param mixed $order
 */
function media_get_list($condition, $num=20, $limit=0, $order=null){
	$limit = $num ? "$limit,$num" : ($limit ? $limit : '');
	!$order && $order = 'id DESC';
	$itemlist = M('media')->where($condition)->limit($limit)->order($order)->select();
	if ($itemlist) {
		return $itemlist;
	}else {
		return array();
