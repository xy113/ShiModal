...
class Application{
	...
	function __construct(){
		...
		//加载function文件
		$functionlist = array('comment', 'member', 'misc', 'photo', 'post');
		...
		*/
		...
		$member = authcode(cookie('user_member'), 1);
		...
		if(!empty($this->var['member'])){
			...
			if ($this->var['uid'] && $this->var['username']){
				...
				$group = authcode(cookie('user_group'), 1);
				$this->var['group'] = $group ? unserialize($group) : array();
		
				$userinfo = authcode(cookie('user_userinfo'), 1);
				$this->var['userinfo'] = $userinfo ? unserialize($userinfo) : array();
		
				$status = authcode(cookie('user_status'), 1);
